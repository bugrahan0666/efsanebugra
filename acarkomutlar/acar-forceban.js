...
exports.run = async (client, message, args) => {
 message.delete()
  ...
  if(args[0] === "liste") {
    message.channel.send(new Discord.RichEmbed().setAuthor(client.user.username + " Force Ban", client.user.avatarURL).setColor("RANDOM").setFooter(message.guild.name, message.guild.iconURL).setTimestamp().addField('Belirlenen Kullanıcılar', kullanıcılar ? kullanıcılar.map(x => x.slice(1)).join('\n') + "\n\nBu kullanıcılar artık sunucuya giremeyecek." : "Bulunmuyor!")).then(msg => msg.delete(10000))
    return
  }
  if(!kullanıcı || isNaN(kullanıcı) || kullanıcı.length > 20 || kullanıcı.length < 10) return message.reply('Force ban atılacak/kaldırılacak kişinin ID numarasını girmelisin! (**liste** yazarak force banlara bakabilirsin)').then(msg => msg.delete(7000))
  if(kullanıcılar && kullanıcılar.some(id => `k${kullanıcı}` === id)) {
    ...
    message.channel.send(`**${kullanıcı}** ID'li kullanıcı artık sunucuya girebilecek!`).then(msg => msg.delete(5000))
  } else {
    ...
    if(message.guild.members.has(kullanıcı)) {
      ...
      await message.guild.members.get(kullanıcı).send(`\`${message.guild.name}\` sunucusundan kalıcı olarak yasaklandın!`).then(msg => msg.delete(10000))
      await message.guild.ban(kullanıcı, {reason: "Forceban"})
    }
    message.channel.send(`**${kullanıcı}** ID'li kullanıcı artık sunucuya giremeyecek!`).then(msg => msg.delete(5000))
  }
};
