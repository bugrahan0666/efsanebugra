...
await member.setNickname(`${acarayarlar.tag} ${acarf(isim)} | ${yas}`)
  ...
  .addField(`🔹 Kayıt Bilgileri`, `▫ Kayıt Yapılan Üyenin İsmi : \`${acarf(isim)}\`\n▫ Kayıt Yapılan Üyenin Yaşı : \`${yas}\`\n▫ Kayıt Yapılan Üyenin Kullanıcı Adı : \`${member.user.tag}\`\n▫ Kayıt Yapılan Üyenin Kullanıcı ID : \`${member.user.id}\``) 
